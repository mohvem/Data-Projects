...
portfolio <- simp_returns_train %>% 
  ...
  ungroup() %>% 
  bind_rows(market_rets %>% mutate(portfolio = "Market",
                                   ret = ret_mkt) %>% ungroup() %>% select(portfolio, ret, date))


### Time Plots
plt <- ggplot(data = portfolio, mapping = aes(x = date,y = ret, color = portfolio)) +
  geom_line() + 
  xlab("Date") +
  ylab("Monthly Return")+
